...
namespace SocialMediaBackend.Modules.AppSubscriptions.Infrastructure.Security;
...
public sealed class PermissionManager(IDbConnectionFactory factory) : IPermissionManager
{
    ...
    public async Task<bool> UserHasPermission(Guid userId, int permissionId, CancellationToken ct = default)
    {
        const string sql = $"""
            SELECT EXISTS (
                SELECT 1
                FROM {Schema.AppSubscriptions}."UserRoles" ur
                JOIN {Schema.AppSubscriptions}."RolePermissions" rp ON ur."RoleId" = rp."RoleId"
